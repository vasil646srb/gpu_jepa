...
def train_on_shard(shard_path, mask_path, encoder, predictor, target_encoder,
    ...
    while step_in_shard < steps_per_shard:
        ...
        with autocast_ctx:
            # Нормализуем pooled (на всякий случай, хотя encoder уже нормализует)
            pooled_ctx = F.normalize(pooled_ctx, p=2, dim=-1)
            pooled_tgt = F.normalize(pooled_tgt, p=2, dim=-1)
